...
africastalking.initialize(username=sandbox_username, api_key=sandbox_api_key)
...
def query_ai_model(prompt):
    ...
    it returns the generated response text.
    """
    try:
...
def ussd_callback(request):
    ...
    if role:
        if role.role == 'student':
            if text == "":
                response = f"CON Welcome back {student.first_name},\n"
                response += "1. AI  Tutor\n"
                ...
                # Send "How to use" infor via SMS
                how_to_use_msg = (
                    "AI Tutor Help:\n"
                    "1. Just send an SMS with your question (e.g., 'What is photosynthesis?').\n"
                    "2. Use commands for specific help:\n"
                    "- 'QUIZEME [topic]' (e.g., QUIZME biology).\n"
                    "- 'GUIDE [topic]' (e.g., GUIDE algebra).\n"
                    "- 'EXPLAIN [topic]' (e.g., EXPLAIN gravity).\n"
                    "3. If a reply ends with (NEXT), send 'NEXT' to get the rest.\n"
                )
                help_msg = ("BrilliantAfrica Tutor Help:\n" 
                            "Just send an SMS with your question  to 74411 (e.g., 'What is photosynthesis?) to get started.")
                send_sms_chunk(phone_number,  help_msg)
                response = "END We've sent an SMS with 'How to use' Instructions."
                ...
                response = "END Under development"
        ...
        # Unregistered User Registration Flows
        if text == "":
...
def sms_reply_handler(request):
    ...
    #     return HttpResponse("Invalid request method.", status=405)
        ...
        # 1. Check if user is asking for the "NEXT" chunk
        if message.upper() == 'NEXT':
            ...
            if session_data:
                ...
                if current_index == len(chunks):
                    # Send next chunk
                    next_chunk = chunks[current_index]
                    
                    if current_index == len(chunks) - 1:
                        # This is the last chunk, no (NEXT) command
                        cache.delete(cache_key)
                    else:
                        # More chunks remain, add (NEXT) and update cache
                        next_chunk += "( NEXT)"
                        session_data['current_index'] += 1
                    cache.set(cache_key, session_data, timeout=3600)
                
                send_sms_chunk(phone_number, next_chunk)
                return HttpResponse("SMS processed", status=200)
            # if no session or session expired, or index is out of bounds
            send_sms_chunk(phone_number, "There are no more messages. Send a new question.")
            return HttpResponse("No active session", status=200)
